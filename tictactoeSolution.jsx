const GridRowComponent = ({ rows }) => {
  const indexArray = [rowNumber, rowNumber + 1, rowNumber + 2];
  return (
    <div>
      {rows.map(rowNumber => {
        return 
        <div className="grid-row">
          {renderRowSquares(indexArray)}
        </div>;
      })}
    </div>
  );
...
const Board = () => {
  ...
  function renderRowSquares(indexArray) {
    return;
    <div>
      {indexArray.map(index => {
        return <Square takeTurn={takeTurn} id={index}></Square>;
      })}
    </div>;
  }
  const rows = [0, 3, 6];
